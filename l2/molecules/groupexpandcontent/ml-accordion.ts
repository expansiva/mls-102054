/// <mls fileReference="_102054_/l2/molecules/groupexpandcontent/ml-accordion.ts" enhancement="_102020_/l2/enhancementAura" />

// =============================================================================
// ML ACCORDION MOLECULE — GLASSMORPHISM (mls-102054)
// =============================================================================
// Skill Group: expand + content
// Mesma molécula/contrato do mls-102040. Lógica intacta (multiple/disabled/loading,
// teclado WAI-ARIA, evento 'toggle'). A aparência (vidro) vive no .less, escopada
// sob a tag. Layout/animação (flex, max-h, transition) permanece em Tailwind.
...
export class MlAccordionMolecule extends MoleculeAuraElement {
  ...
  // Aparência (glass) via classes locais; layout via Tailwind.
  ...
  render() {
    ...
    return html`
      <div class="w-full"> ${this.renderLabel()} ${this.loading ? this.renderLoading() : this.renderSections()} </div>
    `;
  }
  ...
  private renderLoading(): TemplateResult {
    return html`
      <div class="space-y-2">
        ...
        <div class="glass-acc-skeleton h-10 w-full"></div>
        <div class="glass-acc-muted text-xs">${this.msg.loading}</div>
      </div>
    `;
  }
  ...
  private renderSections(): TemplateResult {
    ...
    if (sectionElements.length === 0) {
      return html`<div class="glass-acc-muted text-sm">${this.msg.empty}</div>`;
    }
    return html`<div class="space-y-2">${sectionElements.map((el, index) => this.renderSection(el, index))}</div>`;
  }
