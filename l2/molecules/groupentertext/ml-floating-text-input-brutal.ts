...
export class MlFloatingTextInputBrutal extends MlFloatingTextInputMolecule {
  private gUid = `flt-brutal-${Math.random().toString(36).slice(2, 9)}`;
  ...
  private brutalFeedback(labelId: string): TemplateResult {
  ...
  private brutalInlineLabel(labelText: string, labelId: string): TemplateResult {
  ...
  private brutalFloatingLabel(labelText: string, labelId: string): TemplateResult {
    if (!labelText) return html``;
    return html`<label id="${labelId}" class="brutal-input-floating-label mb-1 block text-xs">${unsafeHTML(labelText)}</label>`;
  }

  private brutalContainerClasses(hasInlineLabel: boolean): string {
    return [
