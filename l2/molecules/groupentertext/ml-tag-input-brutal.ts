...
export class MlTagInputBrutal extends MlTagInputMolecule {
  ...
  private brutalRemoveClasses(): string {
  ...
  private brutalTag(tag: string, index: number): TemplateResult {
    return html`
  ...
  private brutalTags(): TemplateResult {
  ...
  private brutalTextarea(): TemplateResult {
    const placeholderText = this.placeholder || this.gMsg.placeholder;
  ...
  private brutalEditMode(): TemplateResult {
    ...
    return html`
      <div class="${this.brutalContainerClasses()} p-2 min-h-[42px]" @click=${(e: Event) => this.x.handleContainerClick(e)}>
