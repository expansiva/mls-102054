...
export class PasswordStrengthInputBrutal extends PasswordStrengthInputMolecule {
  ...
  private brutalToggleClasses(): string {
  ...
  private brutalBarClasses(level: StrengthLevel): string {
    return ['brutal-pw-bar', `is-${level}`].join(' ');
  }
  private brutalTextClasses(level: StrengthLevel): string {
    return ['brutal-pw-strength-text text-xs font-medium', `is-${level}`].join(' ');
  }
  private brutalCriteriaClasses(isMet: boolean): string {
  ...
  private brutalToggleButton(): TemplateResult {
  ...
  private brutalStrengthIndicator(): TemplateResult {
  ...
  private brutalCriteriaItem(isMet: boolean, label: string): TemplateResult {
  ...
  private brutalErrorMsg(): TemplateResult {
