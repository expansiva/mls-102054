...
export class MlMultiSelectDropdownBrutal extends MultiSelectDropdownMolecule {
  protected portalClassName = 'brutal-msd-portal';
  private bMsg: MessageType = messages.en;
  private bUid = `brutal-msd-${Math.random().toString(36).slice(2)}`;
  private bLabelId = `${this.bUid}-label`;
  private bHelperId = `${this.bUid}-helper`;
  ...
  private brutalHelperOrError(errorMessage: string): TemplateResult {
  ...
  private brutalSelectedTags(items: ParsedItem[], selectedValues: string[]): TemplateResult {
  ...
  private brutalTriggerContent(items: ParsedItem[], selectedValues: string[]): TemplateResult {
  ...
  private brutalOptionButton(item: ParsedItem, isSelected: boolean, isDisabled: boolean): TemplateResult {
    const x = this.x;
    return html`
      <button
  ...
  private brutalOptionList(groups: ParsedGroup[], items: ParsedItem[], selectedValues: string[]): TemplateResult {
  ...
  protected getPortalTemplate(): TemplateResult {
    ...
    return html`
      <div class="brutal-panel w-full" @keydown=${(e: KeyboardEvent) => x.handlePanelKeydown(e)}>
        ${this.searchable
          ? html`
