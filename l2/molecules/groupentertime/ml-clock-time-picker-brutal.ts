...
export class ClockTimePickerBrutal extends ClockTimePickerMolecule {
  ...
  private brutalFormatTime(value: string | null): string {
    if (!value) return this.bMsg.emptyView;
    const parsed = this.x.parseTime(value);
    if (!parsed) return this.bMsg.emptyView;
  ...
  private brutalStepLabel(): string {
    if (this.x.stage === 'hour') return this.bMsg.hour;
    if (this.x.stage === 'minute') return this.bMsg.minute;
  ...
  private brutalOptionClasses(selected: boolean, disabled: boolean): string {
    return [
  ...
  private brutalClockOptions(): TemplateResult {
    const x = this.x;
    if (x.stage === 'hour') {
  ...
  private brutalAmPm(): TemplateResult {
    const x = this.x;
