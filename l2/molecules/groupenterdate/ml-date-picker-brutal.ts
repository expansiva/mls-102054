...
export class MlDatePickerBrutal extends MlDatePickerMolecule {
  protected portalClassName = 'brutal-dp-portal';
  private bMsg: MessageType = messages.en;
  private bFieldId = `dp-brutal-${Math.random().toString(36).slice(2)}`;
  ...
  private brutalDayClasses(isToday: boolean, isSelected: boolean, disabled: boolean): string {
    return [
