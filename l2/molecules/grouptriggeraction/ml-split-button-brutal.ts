...
export class MlSplitButtonBrutal extends MlSplitButtonMolecule {
  ...
  private brutalRenderSpinner(): TemplateResult {
    const sizeMap: Record<string, string> = { xs: 'h-3.5 w-3.5', sm: 'h-4 w-4', md: 'h-4.5 w-4.5', lg: 'h-5 w-5' };
  ...
  private brutalRenderChevron(): TemplateResult {
  ...
  private brutalRenderMenuOptions(items: SecondaryItem[]): TemplateResult {
    const x = this.x;
