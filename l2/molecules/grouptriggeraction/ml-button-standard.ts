/// <mls fileReference="_102054_/l2/molecules/grouptriggeraction/ml-button-standard.ts" enhancement="_102020_/l2/enhancementAura"/>
// =============================================================================
// BUTTON STANDARD MOLECULE — GLASSMORPHISM (mls-102054)
// =============================================================================
// Skill Group: groupTriggerAction
// Mesma molécula/contrato do mls-102040. Lógica intacta (variants, sizes, loading,
// ícone, evento 'action'). A aparência (vidro) vive no .less, escopada sob a tag.
// Layout (flex, padding, gap, tamanho de ícone) permanece em Tailwind.
...
export class ButtonStandardMolecule extends MoleculeAuraElement {
  ...
  // Layout (Tailwind): padding, escala de ícone e gap/text-size por tamanho.
  private getSizeClasses(hasLabel: boolean): { button: string; icon: string; spinner: string; gap: string } {
    ...
    const iconMap: Record<string, string> = { xs: 'h-3 w-3', sm: 'h-4 w-4', md: 'h-5 w-5', lg: 'h-6 w-6' };
    ...
    return { button: paddingMap[sizeKey], icon: iconMap[sizeKey], spinner: iconMap[sizeKey], gap: gapMap[sizeKey] };
  }

  // Aparência (glass): classes locais definidas no .less, dirigidas por variant/estado.
  ...
  private getIconClasses(sizeClass: string): string {
    return ['inline-flex items-center justify-center', sizeClass].join(' ');
  }
  ...
  render() {
    ...
    const spinnerTemplate = html`<span class="${this.getIconClasses(sizeClasses.spinner)}"
      >${this.renderSpinner(sizeClasses.spinner)}</span
    >`;
